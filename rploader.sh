...
#m shell mofified
    ...
    [ ! -f /home/tc/extractor.gz ] && sudo curl -kL --progress-bar "https://raw.githubusercontent.com/PeterSuh-Q3/tinycore-redpill/master/extractor.gz" -o /home/tc/extractor.gz
...
function downloadextractorv2() {
    ...
    curl --insecure --silent --location https://github.com/PeterSuh-Q3/tinycore-redpill/blob/main/tools/xxd?raw=true -o xxd
...
#    loaderdisk=$(mount | grep -i optional | grep cde | awk -F / '{print $3}' | uniq | cut -c 1-3)
    ...
    if [ "${TARGET_PLATFORM}" = "apollolake" ]; then
        dtbfile="ds918p"
        curl --location "https://raw.githubusercontent.com/PeterSuh-Q3/tinycore-redpill/master/ds920p.dts" -o /home/tc/redpill-load/ds918p.dts
    elif [ "${TARGET_PLATFORM}" = "bromolow" ]; then
        dtbfile="ds3615xs"
        curl --location "https://raw.githubusercontent.com/PeterSuh-Q3/tinycore-redpill/master/ds1621p.dts" -o /home/tc/redpill-load/ds3615xs.dts
    elif [ "${TARGET_PLATFORM}" = "broadwell" ]; then
        dtbfile="ds3617xs"
        curl --location "https://raw.githubusercontent.com/PeterSuh-Q3/tinycore-redpill/master/ds1621p.dts" -o /home/tc/redpill-load/ds3617xs.dts
    elif [ "${TARGET_PLATFORM}" = "broadwellnk" ]; then
        dtbfile="ds3622xsp"
        curl --location "https://raw.githubusercontent.com/PeterSuh-Q3/tinycore-redpill/master/ds1621p.dts" -o /home/tc/redpill-load/ds3622xsp.dts
    elif [ "${TARGET_PLATFORM}" = "v1000" ]; then
        dtbfile="ds1621p"
        curl --location "https://raw.githubusercontent.com/PeterSuh-Q3/tinycore-redpill/master/ds1621p.dts" -o /home/tc/redpill-load/ds1621p.dts
    elif [ "${TARGET_PLATFORM}" = "denverton" ]; then
        dtbfile="dva3221"
        curl --location "https://raw.githubusercontent.com/PeterSuh-Q3/tinycore-redpill/master/ds1621p.dts" -o /home/tc/redpill-load/dva3221.dts
    elif [ "${TARGET_PLATFORM}" = "geminilake" ]; then
        dtbfile="ds920p"
        curl --location "https://raw.githubusercontent.com/PeterSuh-Q3/tinycore-redpill/master/ds920p.dts" -o /home/tc/redpill-load/ds920p.dts
    elif [ "${TARGET_PLATFORM}" = "ds923p" ]; then
        dtbfile="ds923p"
        curl --location "https://raw.githubusercontent.com/PeterSuh-Q3/tinycore-redpill/master/ds923p.dts" -o /home/tc/redpill-load/ds923p.dts
    elif [ "${TARGET_PLATFORM}" = "dva1622" ]; then
        dtbfile="dva1622"
        curl --location "https://raw.githubusercontent.com/PeterSuh-Q3/tinycore-redpill/master/ds920p.dts" -o /home/tc/redpill-load/dva1622.dts
    elif [ "${TARGET_PLATFORM}" = "ds2422p" ]; then
        dtbfile="ds2422p"
        curl --location "https://raw.githubusercontent.com/PeterSuh-Q3/tinycore-redpill/master/ds1621p.dts" -o /home/tc/redpill-load/ds2422p.dts
    elif [ "${TARGET_PLATFORM}" = "ds1520p" ]; then
        dtbfile="ds1520p"
        curl --location "https://raw.githubusercontent.com/PeterSuh-Q3/tinycore-redpill/master/ds1621p.dts" -o /home/tc/redpill-load/ds1520p.dts
    else
...
function dont_use_patchdtc() {
    ...
    if [ ! -f ${dtbfile}.dts ]; then
        ...
        curl -kL --progress-bar -O "${dtsfiles}/${dtbfile}.dts"
    fi
...
function gettoolchain() {
    ...
    if [ -f dsm-toolchain.7.0.txz ]; then
        ...
        curl --insecure --progress-bar --location "${TOOLKIT_URL}" -o dsm-toolchain.7.0.txz
    fi
...
function getsynokernel() {
    ...
    if [ -f synokernel.txz ]; then
        ...
        cd /home/tc && curl --insecure --progress-bar --location ${SYNOKERNEL_URL} -o synokernel.txz
        checkfilechecksum synokernel.txz ${SYNOKERNEL_SHA}
        echo "OK, file matches sha256sum, extracting"
  ...
  if [ -f chksum ]; then
    ...
    if [ "$(sha256sum /mnt/${tcrppart}/bzImage-friend | awk '{print $1}')" = "$BZIMAGESHA256" ] && [ "$(sha256sum /mnt/${tcrppart}/initrd-friend | awk '{print $1}')" = "$INITRDSHA256" ]; then
        ...
        if [ $? -ne 0 ]; then
            ...
            for file in $URLS; do curl -kL --progress-bar "$file" -O; done
...
function getlatestrploader() {
    ...
    curl --insecure -s --location "$rploaderfile" -o latestrploader.sh
    curl --insecure -s --location "$modalias3" -o modules.alias.3.json.gz
    [ -f modules.alias.3.json.gz ] && gunzip -f modules.alias.3.json.gz
    curl --insecure -s --location "$modalias4" -o modules.alias.4.json.gz
...
function listextension() {
    ...
    if [ ! -f rpext-index.json ]; then
        curl --insecure --progress-bar --location "${modextention}" -o rpext-index.json
    fi
...
#            sudo curl --location --progress-bar "$URLS" -o /home/tc/custom-module/rp-lkms.zip
...
#  else
    ...
    if [ $MODEL == "FS2500" ]; then
        ...
        sudo curl -kL --progress-bar https://raw.githubusercontent.com/PeterSuh-Q3/redpill-load/master/ext/rp-lkm/redpill-linux-v4.4.180+.ko -o /home/tc/custom-module/redpill.ko
        ...
        sudo curl -kL --progress-bar https://raw.githubusercontent.com/PeterSuh-Q3/redpill-load/master/ext/rp-lkm/redpill-linux-ds1019+-v4.4.180+.ko -o /home/tc/custom-module/redpill.ko

    elif [ $MODEL == "DS723+" ]; then
    
        echo "Downloading peter's ${ORIGIN_PLATFORM} 4.4.180 ${MODEL} redpill.ko ..."
        sudo curl -kL --progress-bar https://raw.githubusercontent.com/PeterSuh-Q3/redpill-load/master/ext/rp-lkm/rp-r1000-4.4.180-prod.ko -o /home/tc/custom-module/redpill.ko
       
    elif [ $MODEL == "DS3615xs" ]; then

        echo "Downloading pocopico's ${ORIGIN_PLATFORM} 3.10.108 redpill.ko ..."
        sudo curl -kL --progress-bar https://raw.githubusercontent.com/pocopico/rp-ext/master/redpillprod/releases/redpill-3.10.108.tgz -o /home/tc/custom-module/redpill.ko.tgz
        sudo tar -zxvf /home/tc/custom-module/redpill.ko.tgz -C /home/tc/custom-module/    

#    elif [ $MODEL == "DVA3219" ]; then
#        echo "Downloading peter's ${ORIGIN_PLATFORM} 4.4.180 ${MODEL} redpill.ko ..."
#        sudo curl -kL --progress-bar https://raw.githubusercontent.com/PeterSuh-Q3/redpill-load/master/ext/rp-lkm/redpill-linux-dva3219-v4.4.180+.ko -o /home/tc/custom-module/redpill.ko
...
#        ;;
    ...
    interactive)
        if [ -f interactive.sh ]; then
            ...
            curl -kL --progress-bar "https://raw.githubusercontent.com/PeterSuh-Q3/tinycore-redpill/$build/interactive.sh" -o interactive.sh
            . ./interactive.sh
            exit 99
        fi
        ;;
